...
public class ClinicRunner {
    ...
    public void doClientAction(int choice) throws IOException {
        switch (choice) {
            ...
            case (2):
                //Add Pet To Client
                pet = client.addPetToClient();
                doClientAction(selectClientActions());
                break;

            case (3):
                //Change Client's Name
                clinic.changeClientName(client);
                doClientAction(selectClientActions());
                break;

            case (4):
                //Select Pet By It's Name
                pet = client.getPetByName();
                if (pet != null) {
